...
class D_praja extends CI_Controller
{
  ...
  function edt($id)
  {
    if ($this->session->userdata('nip') != NULL) {
      ...
      $data = $this->D_praja_model->edit($id)->result();
  ...
  function view_edit()
  {
    if ($this->session->userdata('nip') != NULL) {
      ...
      $input_data['kab_kota'] = $this->input->post('kab_kota', true);
      $input_data['kode_pos'] = $this->input->post('kode_pos', true);
      $input_data['provinsi'] = $this->input->post('provinsi', true);
      $input_data['tlp_pribadi'] = $this->input->post('tlp_pribadi', true);
      $input_data['tlp_rmh'] = $this->input->post('tlp_rmh', true);
      ...
      $result = $this->D_praja_model->view_edit($input_data);
